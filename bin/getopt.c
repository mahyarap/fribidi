	...
	if (!d->__posixly_correct && argc == __libc_argc &&
		...
		if (d->__nonoption_flags_max_len == 0) {
			if (__getopt_nonoption_flags == NULL ||
				...
				__getopt_nonoption_flags = (char *)malloc(
				    d->__nonoption_flags_max_len);
				if (__getopt_nonoption_flags == NULL)
					d->__nonoption_flags_max_len = -1;
				else
					memset(
					    __mempcpy(__getopt_nonoption_flags,
						      orig_str, len),
					    '\0',
...
#define NONOPTION_P                                                            \
	(argv[d->optind][0] != '-' || argv[d->optind][1] == '\0' ||            \
	 (d->optind < d->__nonoption_flags_len &&__getopt_nonoption_flags      \
			  [d->optind] ==                                       \
	  '1'))
#else
#define NONOPTION_P (argv[d->optind][0] != '-' || argv[d->optind][1] == '\0')
	...
	if (longopts != NULL &&
		...
		for (nameend = d->__nextchar; *nameend && *nameend != '=';
		     nameend++)
			/* Do nothing.  */;
		...
		for (p = longopts, option_index = 0; p->name;
		     p++, option_index++)
			if (!strncmp(p->name, d->__nextchar,
