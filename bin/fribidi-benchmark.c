 ...
 * $Revision: 1.8 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/bin/fribidi-benchmark.c,v
 *$
 *
 * Authors:
...
#define TEST_STRING                                                            \
	"a THE QUICK -123,456 (FOX JUMPS ) DOG the quick !1@7#4&5^ over the "  \
	"dog "                                                                 \
...
static void benchmark(const char *S_, int niter)
{
	int len, i;
	...
	for (i = 0; i < niter; i++) {
		...
		if (!fribidi_log2vis(us, len, &base,
				     ...
				     embedding_list))
			die2("something failed in fribidi_log2vis.\n"
			     "perhaps memory allocation failure.",
			     NULL);
	}
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "hVn:", long_options,
				&option_index);
		if (c == -1)
			break;
