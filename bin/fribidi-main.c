 ...
 * $Revision: 1.15 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/bin/fribidi-main.c,v
 *$
 *
 * Authors:
...
static void die2(const char *fmt, const char *arg)
{
...
static void help(void)
{
	...
	printf("Usage: " appname " [OPTION]... [FILE]...\n"
	       ...
	       "  -v, --verbose         Verbose mode, same as --basedir --ltov "
	       "--vtol\n"
	       "                        --levels --changes\n");
	...
	printf("  -c, --charset CS      Specify character set, default is %s.\n"
	       "                        CS should be a valid iconv character "
	       "set name\n",
	       char_set);
#else  /* !FRIBIDI_MAIN_USE_ICONV_H */
	printf(
	    "  -c, --charset CS      Specify character set, default is %s\n"
	    "      --charsetdesc CS  Show descriptions for character set CS "
	    "and exit\n"
	...
	printf("      --showinput       Output the input string too\n"
	       ...
	       "  -w, --width W         Screen width for padding, default is "
	       "%d, but if\n"
	       "                        environment variable COLUMNS is "
	       "defined, its value\n"
	       "                        will be used, --width overrides both "
	       "of them.\n",
	       text_width);
	printf("  -B, --bol BOL         Output string BOL before the visual "
	       "string\n"
	       "  -E, --eol EOL         Output string EOL after the visual "
	       "string\n"
	       "      --rtl             Force base direction to RTL\n"
	       "      --ltr             Force base direction to LTR\n"
	       "      --wrtl            Set base direction to RTL if no strong "
	       "character found\n");
	printf("      --wltr            Set base direction to LTR if no strong "
	       "character found\n"
	       "                        (default)\n"
	       "      --nomirror        Turn mirroring off, to do it later\n"
	       "      --reordernsm      Reorder NSM sequences to follow their "
	       "base character\n"
	       "      --clean           Remove explicit format codes in visual "
	       "string\n"
	       "                        output, currently does not affect "
	       "other outputs\n"
	...
	printf("      --ltov            Output Logical to Visual position map\n"
	       ...
	       "      --novisual        Do not output the visual string, to be "
	       "used with\n"
...
#else
	{
		...
		for (i = 1; i <= FRIBIDI_CHAR_SETS_NUM; i++)
			printf("  * %-10s: %-25s%1s\n",
			       fribidi_char_set_name(i),
			       fribidi_char_set_title(i),
			       (fribidi_char_set_desc(i) ? "X" : ""));
		printf("  X: Character set has descriptions, use --charsetdesc "
		       "to see\n");
	}
#endif /* !FRIBIDI_MAIN_USE_ICONV_H */
...
static char *my_fribidi_strdup(char *s)
{
	char *m;
...
#define CAPRTL 258
	...
	while (1) {
		...
		static struct option long_options[] = {
			...
			{ "ltr",				0,
			  (int *)(void *)&input_base_direction, FRIBIDI_PAR_LTR },
			{ "rtl",				0,
			  (int *)(void *)&input_base_direction, FRIBIDI_PAR_RTL },
			{ "wltr",				0,
			  (int *)(void *)&input_base_direction, FRIBIDI_PAR_WLTR },
			{ "wrtl",				0,
			  (int *)(void *)&input_base_direction, FRIBIDI_PAR_WRTL },
		...
		case 'V':
			...
			break;
		case 'v':
			show_basedir = show_ltov = show_vtol = show_levels =
			    true;
			break;
		case 'w':
		...
		case 'd':
			if (!fribidi_set_debug(true))
				die1("lib" FRIBIDI
				     " must be compiled with DEBUG option to "
				     "enable\nturn debug info on.\n");
			break;
		case 't':
		...
		case CHARSETDESC:
			...
			if (!fribidi_char_set_desc(char_set_num))
				die2("no description available for character "
				     "set `%s'\n",
				...
				printf("Descriptions for character set %s:\n"
				       "\n"
				       "%s",
				       fribidi_char_set_title(char_set_num),
	...
	while (optind < argc || !file_found) {
		...
		/* Read and process input one line at a time */
		{
			...
			padding_width =
			    show_input ? (text_width - 10) / 2 : text_width;
			break_width =
			    do_break ? padding_width : 3 * MAX_STR_LEN;
...
/* TODO: handle \r */
...

				{
					...
					visual =
					    show_visual
						? ALLOCATE(FriBidiChar, len + 1)
						: NULL;
					ltov = show_ltov
						   ? ALLOCATE(FriBidiStrIndex,
							      len + 1)
						   : NULL;
					vtol = show_vtol
						   ? ALLOCATE(FriBidiStrIndex,
...
#else  /* !FRIBIDI_MAIN_USE_ICONV_H */
									new_len = fribidi_unicode_to_charset(
									    char_set_num,
									    visual +
										st,
									    inlen,
