 ...
 * $Revision: 1.7 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/charset/fribidi-char-sets.c,v
 *$
 *
 * Authors:
...
typedef struct
{
	FriBidiChar (*charset_to_unicode_c)(char ch);

	FriBidiStrIndex (*charset_to_unicode)(const char *s,
					      FriBidiStrIndex len,
					      FriBidiChar *us);

	char (*unicode_to_charset_c)(FriBidiChar uch);

	FriBidiStrIndex (*unicode_to_charset)(const FriBidiChar *us,
...
#define _FRIBIDI_ADD_CHAR_SET_ONE2ONE(CHAR_SET, char_set)                      \
	{                                                                      \
		fribidi_##char_set##_to_unicode_c, NULL,                       \
		    fribidi_unicode_to_##char_set##_c, NULL,                   \
...
#define _FRIBIDI_ADD_CHAR_SET_OTHERS(CHAR_SET, char_set)                       \
	{                                                                      \
		NULL, fribidi_##char_set##_to_unicode, NULL,                   \
...
FRIBIDI_ENTRY FriBidiStrIndex fribidi_charset_to_unicode(
    /* input */
    FriBidiCharSet char_set, const char *s, FriBidiStrIndex len,
    /* output */
    FriBidiChar *us)
{
	if (char_sets[char_set].charset_to_unicode)
		...
		for (l = len; l; l--)
			*us++ =
			    (*char_sets[char_set].charset_to_unicode_c)(*s++);
		return len;
	} else
		return 0;
}
...
FRIBIDI_ENTRY FriBidiStrIndex fribidi_unicode_to_charset(
    /* input */
    FriBidiCharSet char_set, const FriBidiChar *us, FriBidiStrIndex len,
    /* output */
    char *s)
{
	if (char_sets[char_set].unicode_to_charset)
		...
		for (l = len; l; l--)
			*s++ =
			    (*char_sets[char_set].unicode_to_charset_c)(*us++);
		*s = '\0';
		return len;
	} else
		return 0;
...
FRIBIDI_ENTRY const char *fribidi_char_set_title(
    ...
    FriBidiCharSet char_set)
{
	return char_sets[char_set].title ? char_sets[char_set].title
