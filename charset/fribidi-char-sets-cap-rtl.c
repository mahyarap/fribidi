 ...
 * $Revision: 1.12 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/charset/fribidi-char-sets-cap-rtl.c,v
 *$
 *
 * Authors:
...
static FriBidiCharType CapRTLCharTypes[] = {
	/* *INDENT-OFF* */
	ON,  ON,  ON,  ON,  LTR, RTL, ON,  ON,
	ON,  ON,  ON,  ON,  ON,  BS,  RLO, RLE, /* 00-0f */
	LRO, LRE, PDF, WS,  ON,  ON,  ON,  ON,
	ON,  ON,  ON,  ON,  ON,  ON,  ON,  ON, /* 10-1f */
	WS,  ON,  ON,  ON,  ET,  ON,  ON,  ON,
	ON,  ON,  ON,  ET,  CS,  ON,  ES,  ES, /* 20-2f */
	EN,  EN,  EN,  EN,  EN,  EN,  AN,  AN,
	AN,  AN,  CS,  ON,  ON,  ON,  ON,  ON, /* 30-3f */
	RTL, AL,  AL,  AL,  AL,  AL,  AL,  RTL,
	RTL, RTL, RTL, RTL, RTL, RTL, RTL, RTL, /* 40-4f */
	RTL, RTL, RTL, RTL, RTL, RTL, RTL, RTL,
	RTL, RTL, RTL, ON,  BS,  ON,  BN,  ON, /* 50-5f */
	NSM, LTR, LTR, LTR, LTR, LTR, LTR, LTR,
	LTR, LTR, LTR, LTR, LTR, LTR, LTR, LTR, /* 60-6f */
	LTR, LTR, LTR, LTR, LTR, LTR, LTR, LTR,
	LTR, LTR, LTR, ON,  SS,  ON,  WS,  ON, /* 70-7f */
	/* *INDENT-ON* */
};
...
static void init_cap_rtl(void)
{
	...
	caprtl_to_unicode = (FriBidiChar *)fribidi_malloc(
	    CAPRTL_CHARS * sizeof caprtl_to_unicode[0]);
	for (i = 0; i < CAPRTL_CHARS; i++)
		if (CapRTLCharTypes[i] == fribidi_get_bidi_type(i)) {
			...
			if (fribidi_get_mirror_char(i, NULL)) {
				DBG("warning: I could not map mirroring "
				    "character map to itself in CapRTL");
			}
	...
	for (i = 0; i < 0x10000 && count;
	     i++) /* Assign BMP chars to CapRTL entries */
		if (!fribidi_get_mirror_char(i, NULL) &&
		    !(i < CAPRTL_CHARS && mark[i])) {
			int j, k;
	...
	if (count) {
		...
		for (j = 0; j < num_types; j++)
			if (request[j]) {
				DBG2("  need this type: %s",
				     fribidi_get_bidi_type_name(to_type[j]));
			}
	}
}

static char fribidi_unicode_to_cap_rtl_c(
    /* input */
    FriBidiChar uch)
{
	int i;
...
FriBidiStrIndex fribidi_cap_rtl_to_unicode(
    /* input */
    const char *s, FriBidiStrIndex len,
    /* output */
    FriBidiChar *us)
{
	FriBidiStrIndex i, j;
...
FriBidiStrIndex fribidi_unicode_to_cap_rtl(
    /* input */
    const FriBidiChar *us, FriBidiStrIndex len,
    /* output */
    char *s)
{
	FriBidiStrIndex i;
	...
	for (i = 0; i < len; i++) {
		...
		if (!FRIBIDI_IS_EXPLICIT(fribidi_get_bidi_type(ch)) &&
			...
			default:
				...
				if (ch < 256)
					s[j++] =
					    fribidi_unicode_to_cap_rtl_c(ch);
				else
					s[j++] = '?';
				break;
			}
