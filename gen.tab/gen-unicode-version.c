 ...
 * $Revision: 1.12 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/gen.tab/gen-unicode-version.c,v
 *$
 *
 * Author:
...
#define READ_VERSION(prefix)                                                   \
	((where = strstr(buf, prefix)) &&                                      \
	 (3 == sscanf(where, prefix "%d.%d.%d", &version_major,                \
...
static int read_data(const char *data_file_type, const char *data_file_name)
{
	FILE *f;
	int status;
...
static void gen_unicode_version(const char *data_file_type)
{
	...
	printf("#define FRIBIDI_UNICODE_VERSION \"%s\"\n"
	       ...
	       version_major, version_minor, version_micro, version_major,
	       version_minor, version_micro);
