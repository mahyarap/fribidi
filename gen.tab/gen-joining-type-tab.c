 ...
 * $Revision: 1.7 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/gen.tab/gen-joining-type-tab.c,v
 *$
 *
 * Author:
...
static void die3(const char *fmt, const char *p, const char *q)
{
	fprintf(stderr, appname ": ");
...
static void die3l(const char *fmt, unsigned long l, const char *p)
{
	fprintf(stderr, appname ": ");
...
static const char *ignored_bidi_types[] = { "BN",  "LRE", "RLE", "LRO",
					    "RLO", "PDF", NULL };
...
static const char *type_is(const char *s, const char *type_list[])
{
...
static void read_unicode_data_txt(FILE *f)
{
	...
	while (fgets(buf, sizeof buf, f)) {
		...
		i = sscanf(s, "%lx;%*[^;];%[^; ];%*[^;];%[^; ]", &c, tp_gen,
			   tp_bidi);
		if (i != 3 || c >= FRIBIDI_UNICODE_CHARS)
			die3l("UnicodeData.txt: invalid input at line %ld: %s",
			      l, s);
...
static void read_arabic_shaping_txt(FILE *f)
{
	...
	while (fgets(buf, sizeof buf, f)) {
		...
		if (i == 2)
			...
			i = sscanf(s, "%lx..%lx ; %*[^;]; %[^; ]", &c, &c2,
				   tp) -
			    1;

		if (i != 2 || c > c2 || c2 >= FRIBIDI_UNICODE_CHARS)
			die3l(
			    "ArabicShaping.txt: invalid input at line %ld: %s",
			    l, s);
...
static void read_data(const char *data_file_type[],
		      const char *data_file_name[])
{
	...
	for (; data_file_name[0] && data_file_type[0];
		...
		if (!(f = fopen(data_file_name[0], "rt")))
			die2("error: cannot open `%s' for reading",
			     data_file_name[0]);
		...
		if (!strcmp(data_file_type[0], "UnicodeData.txt"))
			...
			die2("error: unknown data-file type %s",
			     data_file_type[0]);
...
static void gen_joining_type_tab(int max_depth, const char *data_file_type[])
{
	fprintf(stderr, "Generating `" outputname
			"', it may take up to a few minutes\n");
	printf("/* " outputname "\n * generated by " appname " (" FRIBIDI_NAME
	       " " FRIBIDI_VERSION ")\n"
	       " * from the files %s, %s of Unicode "
	       "version " FRIBIDI_UNICODE_VERSION ". */\n\n",
	...
	if (!pack_table(table, FRIBIDI_UNICODE_CHARS, 1, U, max_depth, 1, names,
...
{
	const char *data_file_type[] = { "UnicodeData.txt", "ArabicShaping.txt",
					 NULL };

	if (argc < 4)
		die3("usage:\n  " appname
		     " max-depth /path/to/%s /path/to/%s [junk...]",
		     data_file_type[0], data_file_type[1]);
