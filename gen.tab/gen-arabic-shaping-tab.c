 ...
 * $Revision: 1.2 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/gen.tab/gen-arabic-shaping-tab.c,v
 *$
 *
 * Author:
...
static const char *arabic_shaping_tags[] = { "isolated", "final", "initial",
					     "medial",   NULL };

static int shape_is(const char *s, const char *type_list[])
{
...
static void read_unicode_data_txt(FILE *f)
{
	...
	while (fgets(buf, sizeof buf, f)) {
		...
		if (i != 3 || c >= FRIBIDI_UNICODE_CHARS ||
		    unshaped >= FRIBIDI_UNICODE_CHARS)
...
static void gen_arabic_shaping_tab(int max_depth /* currently unused */,
				   const char *data_file_type[])
{
	...
	if (maxshaped < minshaped)
		die("error: no shaping pair found, something wrong with "
		    "reading input");
	...
	printf("/*\n"
	       ...
	       " */\n\n",
	       macro_name,
	       (long)(maxshaped - minshaped + 1) * 4 * sizeof(FriBidiChar));
	...
	printf("#define %s(x,shape) "
	       "FRIBIDI_ACCESS_SHAPE_TABLE(%s, 0x%04lx, 0x%04lx, (x), "
	       "(shape))\n\n",
	       macro_name, table_name, (unsigned long)minshaped,
	       (unsigned long)maxshaped);
...
{
	const char *data_file_type[] = { "UnicodeData.txt", NULL };
