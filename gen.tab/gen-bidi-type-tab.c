 ...
 * $Revision: 1.18 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/gen.tab/gen-bidi-type-tab.c,v
 *$
 *
 * Author:
...
static void die3(const char *fmt, unsigned long l, const char *p)
{
	fprintf(stderr, appname ": ");
...
static void init_tab_derived_bidi_class_txt(void)
{
	clear_tab();
}
...
static void read_derived_bidi_class_txt(FILE *f)
{
...
static void read_data(const char *data_file_type, const char *data_file_name)
{
	FILE *f;
...
static void gen_bidi_type_tab(int max_depth, const char *data_file_type)
{
	...
	if (!pack_table(table, FRIBIDI_UNICODE_CHARS, 1, LTR, max_depth, 3,
			names, "unsigned char", table_name, macro_name, stdout))
