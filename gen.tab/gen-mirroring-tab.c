 ...
 * $Revision: 1.14 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/gen.tab/gen-mirroring-tab.c,v
 *$
 *
 * Author:
...
static void die4(const char *fmt, unsigned long l, unsigned long p,
		 unsigned long q)
{
	fprintf(stderr, appname ": ");
...
static void read_bidi_mirroring_txt(FILE *f)
{
	...
	while (fgets(buf, sizeof buf, f)) {
		...
		if (k != 2 || i >= FRIBIDI_UNICODE_CHARS ||
		    j >= FRIBIDI_UNICODE_CHARS)
			die4("invalid pair in input at line %ld: %04lX, %04lX",
			     l, i, j);
...
static void gen_mirroring_tab(int max_depth, const char *data_file_type)
{
	...
	if (!pack_table(table, FRIBIDI_UNICODE_CHARS, key_bytes, 0, max_depth,
			1, NULL, key_type, table_name, macro_name "_DELTA",
			stdout))
