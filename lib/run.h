 ...
 * $Revision: 1.10 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/lib/run.h,v $
 *
 * Authors:
...
FriBidiRun *
new_run(void) FRIBIDI_GNUC_HIDDEN FRIBIDI_GNUC_MALLOC FRIBIDI_GNUC_WARN_UNUSED;

#define free_run FRIBIDI_PRIVATESPACE(free_run)
void free_run(FriBidiRun *run) FRIBIDI_GNUC_HIDDEN;

#define new_run_list FRIBIDI_PRIVATESPACE(new_run_list)
FriBidiRun *new_run_list(void)
    FRIBIDI_GNUC_HIDDEN FRIBIDI_GNUC_MALLOC FRIBIDI_GNUC_WARN_UNUSED;

#define free_run_list FRIBIDI_PRIVATESPACE(free_run_list)
void free_run_list(FriBidiRun *run_list) FRIBIDI_GNUC_HIDDEN;

#define run_list_encode_bidi_types                                             \
	FRIBIDI_PRIVATESPACE(run_list_encode_bidi_types)
FriBidiRun *run_list_encode_bidi_types(const FriBidiCharType *bidi_types,
				       const FriBidiStrIndex len)
    FRIBIDI_GNUC_HIDDEN FRIBIDI_GNUC_WARN_UNUSED;

#define shadow_run_list FRIBIDI_PRIVATESPACE(shadow_run_list)
fribidi_boolean shadow_run_list(FriBidiRun *base, FriBidiRun *over,
				fribidi_boolean preserve_length)
    FRIBIDI_GNUC_HIDDEN FRIBIDI_GNUC_WARN_UNUSED;

#define swap(a, b)                                                             \
	FRIBIDI_BEGIN_STMT void *t;                                            \
	(t) = (a);                                                             \
	(a) = (b);                                                             \
	(b) = (t);                                                             \
	FRIBIDI_END_STMT
...
#define merge_lists(a, b)                                                      \
	FRIBIDI_BEGIN_STMT swap((a)->prev->next, (b)->prev->next);             \
	swap((a)->prev, (b)->prev);                                            \
	FRIBIDI_END_STMT

#define delete_node(x)                                                         \
	FRIBIDI_BEGIN_STMT(x)->prev->next = (x)->next;                         \
	(x)->next->prev = (x)->prev;                                           \
	FRIBIDI_END_STMT

#define insert_node_before(x, list)                                            \
	FRIBIDI_BEGIN_STMT(x)->prev = (list)->prev;                            \
	(list)->prev->next = (x);                                              \
	(x)->next = (list);                                                    \
	(list)->prev = (x);                                                    \
	FRIBIDI_END_STMT
...
#define for_run_list(x, list)                                                  \
	for ((x) = (list)->next; (x)->type != FRIBIDI_TYPE_SENTINEL;           \
	     (x) = (x)->next)

#if DEBUG + 0

#define fribidi_validate_run_list FRIBIDI_PRIVATESPACE(validate_run_list)
void fribidi_validate_run_list(FriBidiRun *run_list /* input run list */
			       ) FRIBIDI_GNUC_HIDDEN;
