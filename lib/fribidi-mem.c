 ...
 * $Revision: 1.8 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/lib/fribidi-mem.c,v
 *$
 *
 * Authors:
...
FriBidiMemChunk *fribidi_mem_chunk_new(
    /* input */
    const char *name, int atom_size, unsigned long area_size, int alloc_type)
{
    ...
    FriBidiMemChunk *mem_chunk)
{
	...
	if
		UNLIKELY(mem_chunk->empty_size < mem_chunk->atom_size)
		{
			register void *chunk =
			    fribidi_malloc(mem_chunk->area_size);
			if
				LIKELY(chunk)
				{
					if (mem_chunk->chunk)
						*(void **)chunk =
						    (char *)mem_chunk->chunk +
						    mem_chunk->empty_size -
						    mem_chunk->area_size;
					chunk = (char *)chunk +
						mem_chunk->atom_size;
    ...
    FriBidiMemChunk *mem_chunk)
{
	...
	chunk = (char *)mem_chunk->chunk + mem_chunk->empty_size -
		mem_chunk->area_size;
	while
		LIKELY(chunk)
		{
