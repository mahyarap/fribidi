 ...
 * $Revision: 1.8 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/lib/fribidi-run.c,v
 *$
 *
 * Authors:
...
void free_run_list(FriBidiRun *run_list)
{
	if (!run_list)
		return;
...
fribidi_boolean shadow_run_list(
    /* input */
    FriBidiRun *base, FriBidiRun *over, fribidi_boolean preserve_length)
{
	...
	{
		if
			UNLIKELY(!q->len || q->pos < pos) continue;
		pos = q->pos;
		while (p->next->type != FRIBIDI_TYPE_SENTINEL &&
		       p->next->pos <= pos)
			p = p->next;
		/* now p is the element that q must be inserted 'in'. */
		pos2 = pos + q->len;
		r = p;
		while (r->next->type != FRIBIDI_TYPE_SENTINEL &&
		       r->next->pos < pos2)
			r = r->next;
		if (preserve_length)
			r->len += q->len;
		...
		if
			LIKELY(p == r)
			{
				/* split p into at most 3 intervals, and insert
				   q in the place of
				   the second interval, set r to be the third
				   part. */
				/* third part needed? */
				if (p->pos + p->len > pos2) {
					r = new_run();
		...
		/* before updating the next and prev runs to point to the
		   inserted q,
