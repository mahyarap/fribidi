 ...
 * $Revision: 1.3 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/lib/fribidi-arabic.c,v
 *$
 *
 * Author(s):
...
static void fribidi_shape_arabic_joining(
    /* input */
    const FriBidiChar table[][4], FriBidiChar min, FriBidiChar max,
    const FriBidiStrIndex len, const FriBidiArabicProp *ar_props,
    /* input and output */
    FriBidiChar *str)
{
	...
	for (i = 0; i < len; i++)
		if (FRIBIDI_ARAB_SHAPES(ar_props[i]))
			str[i] = FRIBIDI_ACCESS_SHAPE_TABLE_REAL(
			    table, min, max, str[i],
			    FRIBIDI_JOIN_SHAPE(ar_props[i]));
}
...
static int comp_PairMap(const void *pa, const void *pb)
{
	...
	if (a->pair[0] != b->pair[0])
		...
		return a->pair[1] < b->pair[1] ? -1 : a->pair[1] > b->pair[1]
							  ? +1
							  : 0;
}

static FriBidiChar find_pair_match(const PairMap *table, int size,
				   FriBidiChar first, FriBidiChar second)
{
	PairMap *match;
	PairMap x;
	x.pair[0] = first;
...
#define PAIR_MATCH(table, len, first, second)                                  \
	((first) < (table[0].pair[0]) || (first) > (table[len - 1].pair[0])    \
	     ? 0                                                               \
	     : find_pair_match(table, len, first, second))

static void fribidi_shape_arabic_ligature(
    /* input */
    const PairMap *table, int size, const FriBidiLevel *embedding_levels,
...
#define DO_LIGATURING(table, levels, len, ar_props, str)                       \
	fribidi_shape_arabic_ligature((table),                                 \
				      sizeof(table) / sizeof((table)[0]),      \
				      levels, len, ar_props, str)
...
FRIBIDI_ENTRY void fribidi_shape_arabic(
    /* input */
    FriBidiFlags flags, const FriBidiLevel *embedding_levels,
    const FriBidiStrIndex len,
    /* input and output */
    FriBidiArabicProp *ar_props, FriBidiChar *str)
{
