 ...
 * $Revision: 1.6 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/lib/fribidi-deprecated.c,v
 *$
 *
 * Authors:
...
{
	return FRIBIDI_ADJUST_AND_TEST_BITS(flags, FRIBIDI_FLAG_SHAPE_MIRRORING,
					    state);
}

FRIBIDI_ENTRY fribidi_boolean fribidi_mirroring_status(void)
{
...
FRIBIDI_ENTRY fribidi_boolean fribidi_set_reorder_nsm(
    ...
    fribidi_boolean state)
{
	return FRIBIDI_ADJUST_AND_TEST_BITS(flags, FRIBIDI_FLAG_REORDER_NSM,
					    state);
}
...
{
	return fribidi_get_par_embedding_levels(bidi_types, len, pbase_dir,
						embedding_levels);
}

FRIBIDI_ENTRY FriBidiCharType
fribidi_get_type(FriBidiChar ch /* input character */
		 )
{
...
FRIBIDI_ENTRY FriBidiCharType
fribidi_get_type_internal(FriBidiChar ch /* input character */
			  )
{
	return fribidi_get_bidi_type(ch);
}
...
fribidi_remove_bidi_marks(FriBidiChar *str, const FriBidiStrIndex len,
			  ...
			  FriBidiLevel *embedding_levels)
{
	...
	if (positions_to_this && !position_from_this_list) {
		position_from_this_list =
		    fribidi_malloc(sizeof(position_from_this_list[0]) * len);
		if
			UNLIKELY(!position_from_this_list) goto out;
	...
	for (i = 0; i < len; i++)
		if (!FRIBIDI_IS_EXPLICIT_OR_BN(fribidi_get_bidi_type(str[i])) &&
			...
			if (position_from_this_list)
				position_from_this_list[j] =
				    position_from_this_list[i];
			j++;
		}
    ...
    FriBidiStrIndex *positions_V_to_L, FriBidiLevel *embedding_levels)
{
	...
	if (!embedding_levels) {
		embedding_levels =
		    fribidi_malloc(len * sizeof embedding_levels[0]);
		if (!embedding_levels)
			goto out;
	...
	if (visual_str) {
		...
		*/
		...
		fribidi_join_arabic(bidi_types, len, embedding_levels,
				    ar_props);

		fribidi_shape(flags, embedding_levels, len, ar_props,
			      visual_str);
	}
	...
	status = fribidi_reorder_line(flags, bidi_types, len, 0, *pbase_dir,
