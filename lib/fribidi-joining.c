 ...
 * $Revision: 1.6 $
 * $Source:
 * /home/behdad/src/fdo/fribidi/togit/git/../fribidi/fribidi2/lib/fribidi-joining.c,v
 *$
 *
 * Authors:
...
static void print_joining_types(
    /* input */
    const FriBidiLevel *embedding_levels, const FriBidiStrIndex len,
    const FriBidiJoiningType *jtypes)
{
	...
	MSG("  Join. types: ");
	for (i = 0; i < len; i++)
		MSG2("%c", fribidi_char_from_joining_type(
			       jtypes[i],
...
#define FRIBIDI_CONSISTENT_LEVEL(i)                                            \
	(FRIBIDI_IS_EXPLICIT_OR_BN(bidi_types[(i)]) ? FRIBIDI_SENTINEL         \
						    : embedding_levels[(i)])
    ...
    FriBidiArabicProp *ar_props)
{
	...
	/* The joining algorithm turned out very very dirty :(.  That's what
	 * happens
	...
	{
		/* The following do not need to be initialized as long as joins
		 * is
		 * initialized to false.  We just do to turn off compiler
		 * warnings. */
		...
		for (i = 0; i < len; i++)
			if (!FRIBIDI_IS_JOINING_TYPE_G(ar_props[i])) {
				...
				register fribidi_boolean shapes =
				    FRIBIDI_ARAB_SHAPES(ar_props[i]);
				register FriBidiLevel level =
				    FRIBIDI_CONSISTENT_LEVEL(i);

				if (joins &&
				    !FRIBIDI_LEVELS_MATCH(saved_level, level)) {
					disjoin = true;
					joins = false;
				}

				if (!FRIBIDI_IS_JOIN_SKIPPED(ar_props[i])) {
					register const FriBidiArabicProp
					...
					if (!joins) {
						if (shapes)
							FRIBIDI_UNSET_BITS(
							    ar_props[i],
							    joins_preceding_mask);
					} else if (!FRIBIDI_TEST_BITS(
							ar_props[i],
							joins_preceding_mask)) {
						disjoin = true;
					} else {
						register FriBidiStrIndex j;
						/* This is a FriBidi extension:
						 * we set joining properties
						 * for skipped characters in
						 * between, so we can put NSMs
						 * on tatweel
						 * later if we want.  Useful on
						 * console for example.
						 */
						for (j = saved + 1; j < i; j++)
							FRIBIDI_SET_BITS(
				...
				if (!FRIBIDI_IS_JOIN_SKIPPED(ar_props[i])) {
					...
					joins = FRIBIDI_TEST_BITS(
					    ar_props[i],
					    saved_joins_following_mask);
				}
			}
		if ((joins) && saved_shapes)
			FRIBIDI_UNSET_BITS(ar_props[saved],
					   saved_joins_following_mask);
	}
